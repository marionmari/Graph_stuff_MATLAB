...
function [IDX,C,sumd] = KmeansPiotrDollar( X,k,varargin )
    ...
    [errmsg,replicates,maxiter,display,randstate,outlierfrac,minCsize] = ...
                                                    GetArgsPiotrDollar(pnames, dflts, varargin{:});
    error(errmsg);
...
function [IDX,C,sumd,niters] = kmeans2_main(X,k,noutliers,minCsize,maxiter,display)    
    ...
    index = RandpermPiotrDollar(N,k);  C = X(index,:); 
    ...
    while( sum(abs(oldIDX - IDX)) ~= 0 && niters < maxiter)
        ...
        oldIDX = IDX;  D = DistEuclideanPiotrDollar( X, C ); [mind IDX] = min(D,[],2);  
        ...
        if( k==0 ) IDX( RandintPiotrDollar( 1,1, [1,N] ) ) = 1; k=1; end;
